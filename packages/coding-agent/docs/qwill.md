...
Qwill is this fork's name for pi running against a local Qwen model. It is implemented as a hidden built-in extension, `qwill`, that tunes the agent for Qwen models served locally through an OpenAI-compatible endpoint (llama.cpp `llama-server`, and similar). It changes nothing for other models.
...
`QWILL_PROFILE` overrides the model check:
